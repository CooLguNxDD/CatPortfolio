...
const FRAGMENT = /* glsl */ `
  ...
  void main() {
    ...
    // Fresnel — glancing angles pick up the sun tint, top-down stays water-coloured.
    float fres = pow(1.0 - clamp(dot(n, v), 0.0, 1.0), 3.0);

    // Blinn specular off the wave normal.
    ...
    float alpha = clamp(uOpacity + fres * 0.25 + spec * 0.35, 0.0, 1.0);
