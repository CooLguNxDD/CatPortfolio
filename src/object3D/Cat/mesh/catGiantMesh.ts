 ...
 *
 * Perched on the rim of the fish tank, overlooking the aquatic domain.
 * Features:
 * - Natural plush "teddy cat" aesthetics: warm mocha coat, marshmallow chest & muzzle, rounded cheeks.
 * - Glowing Yellow Eyes & Dilating Pupils with Forward-Kinematics gaze tracking.
 * - 7-segment fluid tail, articulated swat paw, breathing chest, ear twitching.
 */
...
export function stepCatAnimation(
  ...
  // When target is in front (dz > 0), visibility is 1.0.
  // As target moves into rear hemisphere (dz < 0), visibility smoothly tapers to 0.0 at direct rear.
  // This eliminates extreme head snapping and jitter when orbiting the camera behind the cat.
  const forwardCos = horizontalDist > 0.001 ? dz / horizontalDist : 1.0
  const fovFactor = Math.max(0, Math.min(1, (forwardCos + 0.35) / 0.7))
  const visibility = fovFactor * fovFactor * (3 - 2 * fovFactor) // smoothstep

  // Head yaw & pitch
  const rawYaw = Math.atan2(dx, Math.max(0.001, dz))
  const MAX_ROTATION_RAD = Math.PI / 2 // ±90 degrees
  const clampedYaw = Math.max(-MAX_ROTATION_RAD, Math.min(MAX_ROTATION_RAD, rawYaw)) * visibility
  const rawPitch = -Math.atan2(dy, Math.max(1, horizontalDist))
  const clampedPitch = Math.max(-MAX_ROTATION_RAD, Math.min(MAX_ROTATION_RAD, rawPitch)) * visibility

  parts.headPivot.rotation.y = clampedYaw
  parts.headPivot.rotation.x = clampedPitch + Math.sin(t * 1.8) * 0.02 * visibility
  parts.headPivot.rotation.z = Math.sin(t * 1.1) * 0.03 * visibility - clampedYaw * 0.15
  ...
  // 4. Standout Feature: Glowing Yellow Eyes & Growing/Dilating Pupils
  state.blinkTimer -= dt
  ...
  // Pupil micro-shift inside eye socket towards gaze direction
  const eyeShiftX = Math.max(-0.16, Math.min(0.16, (rawYaw - clampedYaw) * 0.25 + rawYaw * 0.08)) * visibility
  const eyeShiftY = Math.max(-0.16, Math.min(0.16, (rawPitch - clampedPitch) * 0.25 + rawPitch * 0.08)) * visibility

  parts.pupilL.position.x = eyeShiftX
  parts.pupilL.position.y = eyeShiftY
  parts.pupilL.scale.x = state.pupilDilation
  parts.pupilL.scale.y = Math.min(0.95, 0.75 + state.pupilDilation * 0.2)

  parts.pupilR.position.x = eyeShiftX
  parts.pupilR.position.y = eyeShiftY
