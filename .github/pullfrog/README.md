...
Pullfrog is an autonomous AI coding agent integrated into CatPortfolio CI. It runs
...
2.  **[`build.md`](instructions/build.md)**: Governs task execution and coding. Defines SPA structure, npm scripts, layout pipeline, and safety requirements.
...
```sh
gh variable set PULLFROG_REVIEW_BUDGET --body max --repo CooLguNxDD/CatPortfolio
```
...
4.  **No Default Branch Pushing**: Pullfrog is restricted to feature branches. It cannot push directly to `main`. Never touch `.github/workflows/deploy.yml`.
...
| **`PULLFROG_MODEL`** (Var) | `opencode/muse-spark-1.3-contributor-free` | Model slug to run (`opencode/muse-spark-1.3-contributor-free`, `openrouter/meta/muse-spark-1.3-contributor`). |
